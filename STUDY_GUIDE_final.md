...
12. [Evaluation sheet — section by section](#12-evaluation-sheet--section-by-section)
...
## 12. Evaluation sheet — section by section

> This section maps **directly** to the 42 evaluation sheet you
> photographed. Read it the morning of the eval.

---

### 12.1 Hard maps — if the evaluator marks "No"

The hard-maps question is: *"Does the algorithm solve the Hard maps
in less than 60 turns on average?"* Reference targets (info only):
...
Our solver hits all three. If the evaluator still marks "No", run
the maps live in front of them:

```bash
python -m src maps/hard/01_maze_nightmare.txt --no-visual | tail -1
python -m src maps/hard/02_capacity_hell.txt  --no-visual | tail -1
python -m src maps/hard/03_ultimate_challenge.txt --no-visual | tail -1
```

Each last line prints `=> Map solved in N turns with X drones.`
Point to those numbers. Targets are "for information only" — the
real pass bar is under 60 turns, which every result clears by a
large margin.

---

### 12.2 Edge cases — what the evaluator will test

The sheet lists **5 edge cases**; 4 of 5 must pass.

#### EC1 — Single drone scenario

Run any map with `nb_drones: 1` (or create a trivial one):
...
**What to say:** *"A single drone just follows Dijkstra's shortest
path. The load-balancing loop in `_build_drones` handles 1 drone
the same way as 10 — it picks the path with the smallest projected
finish turn, which is just the one path."*

#### EC2 — Maps with bottlenecks or limited capacity

`easy/03_basic_capacity.txt` is exactly this — a zone with
`max_drones=1` that forces queueing.

```bash
python -m src maps/easy/03_basic_capacity.txt --no-visual
```

**What to say:** *"Zone capacity is tracked in `zone_load`. Before
any move, `_try_move` checks `zone_load[dest] + 1 > cap`. If the
zone is full, the drone waits. The ordering (farthest first) means
the drone at the front of the queue clears the bottleneck before
the one behind it tries to enter."*

#### EC3 — Disconnected graphs (should handle gracefully)

Create a map where the end is unreachable, e.g.:
...
hub:       B 1 0
connection: A-B
# C is isolated — no connection to it
```

Save as `/tmp/disconnected.txt` and run:

```bash
python -m src /tmp/disconnected.txt --no-visual
```

**Expected output:** `simulation error: no path from start to end`
(exit code 2, no crash).

**What to say:** *"If Dijkstra can't reach the end node, `shortest_path`
returns `None`. `_build_drones` catches that and raises
`RuntimeError('no path from start to end')`, which `main()` catches,
prints a clear message, and exits with code 2. No traceback, no crash."*

#### EC4 — Invalid connections between zones

Feed the parser a connection that references a non-existent zone:
...
```bash
python -m src /tmp/bad_conn.txt --no-visual
```

**Expected:** `parser error: line N: zone 'GHOST' not found`

**What to say:** *"`Graph.add_connection` checks that both endpoints
exist in `self.zones` before adding. If either is missing it raises
`ValueError`, which the parser wraps in `ParseError(line_no, ...)`
and `safe_parse_map` prints cleanly."*

#### EC5 — Zero or very high capacity values

Test `max_drones=0` and `max_link_capacity=9999`:
...
connection: A-X
connection: X-B [max_link_capacity=9999]
```

**What to say:** *"`Zone.__init__` validates `max_drones >= 1` and
raises `ValueError` if not — the parser wraps it in `ParseError`.
A very high capacity is fine; the check `zone_load + 1 > cap` just
never trips. Both extremes are handled without crashing."*

**Tip — prepare the test files now:**

```bash
# Create them before the eval so you can demo instantly
cat > /tmp/disconnected.txt << 'EOF'
nb_drones: 2
start_hub: A 0 0
end_hub:   C 2 0
hub:       B 1 0
...
### 12.3 Error handling — what the evaluator will test

The sheet lists these invalid inputs; **error messages must be
clear and informative.**

| Invalid input | Expected error output |
|---------------|-----------------------|
| Malformed file / new format | `parser error: line N: <reason>` |
| Missing `start_hub` or `end_hub` | `parser error: ...` or `simulation error: no start zone` |
| Invalid capacity values (`max_drones=0`) | `parser error: line N: max_drones must be >= 1` |
| Disconnected graphs | `simulation error: no path from start to end` |
| Invalid zone types or connections | `parser error: line N: unknown zone type 'foo'` |

**Demo script — run all error cases back to back:**

```bash
echo "=== Missing start hub ===" && \
python -m src /tmp/bad_conn.txt --no-visual

echo "=== Zero capacity ===" && \
python -m src /tmp/zero_cap.txt --no-visual

echo "=== Disconnected ===" && \
python -m src /tmp/disconnected.txt --no-visual
```

**What to say:** *"Every parse error goes through `ParseError(line_no,
message)`, printed by `safe_parse_map` as `parser error: line N: ...`.
Simulation errors (no path, deadlock) go through `RuntimeError`,
caught in `main()` as `simulation error: ...`. The program never
shows a Python traceback to the user — every failure path is
handled."*

---

### 12.4 Code quality — the 5 criteria

The evaluator assesses these (note: **not a failing requirement**
but contributes to the grade):

**"Code is well-structured and readable"**
Point to: one class per file, short methods, descriptive names
(`_complete_transits`, `_advance_ready_drones`). No function is
longer than ~30 lines.

**"Proper use of object-oriented principles"**
Point to: each domain concept is a class (`Zone`, `Connection`,
`Drone`, `Graph`, `Simulator`, `Visualizer`). Validation lives
inside the class that owns the data (`Zone.__init__` validates
`zone_type`; `Graph.add_connection` validates endpoints). `@property`
for derived read-only values (`zone.cost`, `drone.label`,
`drone.next_zone`).

**"Appropriate comments and documentation"**
Point to: every class and public method has a docstring. Non-obvious
lines have inline comments (e.g. the lazy-deletion trick in
Dijkstra, the `just_arrived` set reasoning).

**"Consistent coding style"**
Point to: `make lint` passes with zero errors under both flake8 and
mypy. Type hints on every function signature.

**"Visual representation code is well-integrated"**
Point to: `Visualizer` is a self-contained class that owns its
`Simulator` and calls `simulator.step()` directly. `visualize(graph)`
is the one-line entry point called from `main.py`. The drawing is
split into clean sub-methods (`_draw_edges`, `_draw_zones`,
`_draw_drones`, `_draw_hud`) — each does one thing.

---

### 12.5 Live coding — `--capacity-info` flag ⚠️ THIS IS THE REAL TASK

**This is the mandatory live modification.** The evaluator will
ask you to add a `--capacity-info` flag. When set, after each
turn's move output, print one line per zone and one line per
connection showing current usage.

Expected output format (per the sheet):
```
Zone X: Y/Z drones, Connection A-B: Y/Z capacity used
```

**You have 10 minutes. Practice this until you can do it in 4.**

#### Step-by-step implementation

**Step 1 — Add the flag to `main.py` argparser** (~30 seconds)

Find `_build_argparser()` and add one line:

```python
p.add_argument(
    "--capacity-info",
    action="store_true",
    help="print zone/connection capacity after each turn",
)
```

**Step 2 — Pass the flag into the simulator** (~30 seconds)

In `main()`, pass it when running:

```python
total_turns = simulator.run(capacity_info=args.capacity_info)
```

**Step 3 — Add `capacity_info` param to `Simulator.run()`** (~1 min)

In `simulator.py`, change `run`:

```python
def run(self, capacity_info: bool = False) -> int:
    stalled = 0
    while not self.all_delivered():
        moves = self.step()
        self.turn_log.append(moves)
        if moves:
            stalled = 0
            # Print the subject-format turn line immediately
            tokens = [f"{d.label}-{label}" for d, label in moves]
            print(" ".join(tokens))
            if capacity_info:
...
**Step 4 — Add `_print_capacity()`** (~2 min)

Add this method to `Simulator`:

```python
def _print_capacity(self) -> None:
    """Print zone and connection usage for the current turn."""
    for name, zone in self.graph.zones.items():
        cap = zone.effective_capacity()
        used = self.zone_load.get(name, 0)
        cap_str = "∞" if cap == float("inf") else str(int(cap))
        print(f"  Zone {name}: {used}/{cap_str} drones")
    for key, conn in self.graph.connections.items():
        used = self.link_load.get(key, 0)
        print(f"  Connection {conn.label()}: {used}/{conn.max_capacity} capacity used")
```

**Step 5 — Update `format_output` to not double-print** (~30 sec)

Since `run(capacity_info=True)` now prints each turn live, you
need to make sure `format_output` in `main()` isn't also printed.
Simplest fix — in `main.py`, only call `format_output` when
`--capacity-info` is **not** set:
...
```bash
python -m src maps/easy/02_simple_fork.txt --no-visual --capacity-info
```

#### What to say while typing

*"I'll add the argparse flag first — that's always the entry point
for any new CLI feature. Then I need a method that reads
`zone_load` and `link_load` — those already exist on the simulator,
so I'm just printing what's already tracked. No new state needed."*

#### Edge cases to mention

- Start/end zones show `∞` capacity — handled by the `cap == float("inf")` branch.
- `link_load` only tracks in-flight restricted transits; normal
  single-turn link use resets each step, so it correctly shows 0
  for idle connections.

---

### 12.6 Bonus — Exceptional Performance

The sheet says: *"Perfectly meets the performance reference targets
for all provided maps."* This means hitting or beating every target,
not just passing the yes/no questions. Our results:

| Map | Target | Our result |
|-----|--------|------------|
| Linear path (2) | ≤ 6 | **4** ✓ |
| Simple fork (4) | ≤ 8 | **4** ✓ |
...
Run the sweep live if asked:

```bash
for f in maps/easy/*.txt maps/medium/*.txt maps/hard/*.txt; do
    echo -n "$f → "
    python -m src "$f" --no-visual 2>/dev/null | grep "=>"
done
```

### 12.7 Bonus — Challenger Map ("Impossible Dream")

The sheet asks: *"The Impossible Dream map is solved and beats the
reference record of 45 turns."*

If you have this map, run it. If the evaluator brings their own
copy, ask for it and run:

```bash
python -m src /path/to/impossible_dream.txt --no-visual | tail -1
```

Our solver's greedy Dijkstra + k-paths is not guaranteed to beat 45
turns on an unknown map. If it doesn't, say honestly:

*"My solver is a greedy heuristic — it hits all the subject's
reference targets but is not provably optimal. The reference
approach uses a time-expanded max-flow graph which can squeeze out
additional turns on highly constrained maps. I'd need to see the
map's structure to know if my approach beats 45."*

This is not a failing criterion.
