...
**Job:** show the graph and step through the simulation visually.

```python
COLOR_MAP = {"red": (220, 40, 40), "green": (40, 200, 80), ...}
```

Lookup from color name (in the map metadata) to RGB tuple. Unknown
colors fall back to gray.

```python
class Visualizer:
    WIDTH = 1100; HEIGHT = 720; PADDING = 80
    AUTO_DELAY_MS = 500
```

Constants for the window. `AUTO_DELAY_MS` is how often auto-play
advances (500 ms = 2 turns/sec).
...
Project the map's `(x, y)` integer coordinates onto pixel
coordinates that fit our window. We normalize to `[0, 1]` then
multiply by usable width/height.
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT: running = False
                    ...
                    elif event.key in (pygame.K_SPACE, pygame.K_RIGHT): self._do_step()
                    elif event.key == pygame.K_a: self._auto = not self._auto
                    elif event.key == pygame.K_r: self.simulator = Simulator(...)
...
The classic pygame game loop: handle events, update state, draw,
flip the buffer, throttle to 30 fps.

The drawing methods (`_draw_edges`, `_draw_zones`, `_draw_drones`,
`_draw_hud`) just blit lines/circles/text using pygame primitives.
...
**A:** "The `Visualizer` owns its own `Simulator` instance. Each
time the user presses SPACE (or auto-play fires), the visualizer
calls `simulator.step()` and appends the returned moves to the
turn log. The window redraws based on each drone's current
`position` / `in_flight_to` attributes — drones in flight are
drawn at the midpoint of the connection."
...
for pos, ids in clusters.items():
    color = DRONE_COLOR
    # check if any drone in this cluster is in flight
    for d in self.simulator.drones:
        if d.id in ids and d.in_flight_to is not None:
